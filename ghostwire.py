...
# ── lightweight stubs for CI / unit-tests (set GW_TEST_STUB=1) ─────────
...
# ── packet sniffer (dashboard rewritten) ───────────────────────────────
def _build_sniff_dash(total: int, rate: float) -> Table:
    tbl = Table.grid()
...
def sniff_packets(
    ...
    def _update(pkt) -> None:
        ...
        live.update(_build_sniff_dash(total, rate))

    console.rule("[bright_magenta]Sniffing – Ctrl-C to stop")
    with Live(_build_sniff_dash(0, 0), refresh_per_second=2, console=console) as live:
        try:
            sniff(
                iface=iface,
                prn=_update,
...
def port_scan(target: str, top: int, udp: bool) -> None:
    ...
    out = run_cmd(
        ["nmap", "-T4", flag, f"--top-ports={top}", target],
        capture=True,
    )
...
def _menu() -> None:
    ...
    console.print(
        Panel(g, title="[bright_magenta]Main Menu[/]", border_style="bright_magenta")
    )
...
def interactive() -> None:
    while True:
        ...
        if ch == "1":
            ...
            nmap_scan(
                console.input("Target (IP/host) > ").strip(),
                console.input('nmap options (e.g. "-sV -A -Pn") > ').strip()
                or "-sV -T4",
            )
            ...
            auto_crack(
                console.input("Capture file > "), console.input("Wordlist path > ")
            )
