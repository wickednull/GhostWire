#!/usr/bin/env python3
# BlackLace v6.0 (created by. Null_Lyfe)
# DISCLAIMER: For educational and authorized security testing only.
# Unauthorized use is illegal. Always have explicit permission.
...
def main():
    ...
    menu_options = {
        ...
        "9": ("Exploit Search", searchsploit_exploit),
        "10": ("Metasploit Runner", launch_metasploit_module),
        "11": ("Netcat Listener", start_listener),
        "12": ("Password Cracker", crack_hashes),
        "13": ("Linux PrivEsc Check", run_privesc_check),
        "14": ("Keylogger", start_keylogger),
        "15": ("Webcam Snap", snap_webcam),
        "16": ("Add Startup Persistence", add_startup_persistence),
        "17": ("BLE Jammer", ble_jammer),
        "18": ("Zigbee Sniffer", zigbee_sniff_with_hackrf),
        "0": ("Exit", sys.exit)
    }

    try:
        while True:
            console.print("\n[bold magenta]BlackLace v6.0 (Complete) - Offensive Security Toolkit[/bold magenta]")
